<?php 
/** 
 * SMW_phpSmug - SMW_phpSmug is a PHP wrapper class for the SmugMug API. The intention 
 ...
 * 
 * This file is part of SMW_phpSmug.
 *
 * SMW_phpSmug is free software: you can redistribute it and/or modify
 * it under the terms of the GNU General Public License as published by
 * the Free Software Foundation, either version 3 of the License, or
 * (at your option) any later version.
 *
 * SMW_phpSmug is distributed in the hope that it will be useful,
 * but WITHOUT ANY WARRANTY; without even the implied warranty of
 * MERCHANTABILITY or FITNESS FOR A PARTICULAR PURPOSE.  See the
 * GNU General Public License for more details.
 *
 * You should have received a copy of the GNU General Public License
 * along with SMW_phpSmug.  If not, see <http://www.gnu.org/licenses/>.
 *
 *
 ...
 * 
 * SMW_phpSmug is inspired by phpFlickr 2.1.0 ({@link http://www.phpflickr.com}) by Dan Coulter
 * 
 * Please help support the maintenance and development of SMW_phpSmug by making
 ...
 */
class SMW_PhpSmugException extends Exception {}

/**
 * SMW_phpSmug - all of the SMW_phpSmug functionality is provided in this class
 *
 * @package SMW_phpSmug
 **/
class SMW_phpSmug {
	...
	/**
	 * Constructor to set up a SMW_phpSmug instance.
	 * 
     ...
     * 
     * By default SMW_phpSmug will use the latest stable API endpoint, but 
	...
	{
		$args = SMW_phpSmug::processArgs( func_get_args() );
		...
		// All calls to the API are done via POST using my own constructed SMW_httpRequest class
		$this->req = new SMW_httpRequest();
		...
		$this->req->setHeader( array( 'User-Agent' => "{$this->AppName} using SMW_phpSmug/{$this->version}", 'Content-Type' => 'application/x-www-form-urlencoded' ) );
    }
	...
	/**
	 * General debug function used for testing and development of SMW_phpSmug.
	 *
	 ...
	 *
	 * SMW_phpSmug uses the PEAR MDB2 module to interact with the database. You will
	...
	public function enableCache()
	{
        ...
        if ( $this->cacheType == 'db' ) {
			...
			$options = array( 'comment' => 'SMW_phpSmug cache', 'charset' => 'utf8', 'collate' => 'utf8_unicode_ci' );
			...
			if ( file_exists( $args['cache_dir'] ) && ( is_dir( $args['cache_dir'] ) ) ) {
				$this->cache_dir = realpath( $args['cache_dir'] ).'/SMW_phpSmug/';
	 ...
	 * @param	array		$request Request to the SmugMug created by one of the later functions in SMW_phpSmug.
	 **/
    private function getCached( $request )
	{
		...
		if ( $this->cacheType == 'db' ) {
			...
			if ( PEAR::isError( $result ) ) {
				throw new SMW_PhpSmugException( $result );
			}
			if ( !empty( $result ) ) {
                return $result;
            }
	 ...
	 *						later functions in SMW_phpSmug.
	...
	private function request( $command, $args = array() )
	{
		if ( ( strpos( $command, 'login.with' ) || strpos( $command, 'Token' ) ) || ( $this->oauth_signature_method == 'PLAINTEXT' ) || $this->secure ) {
			...
			if ( ( isset( $this->SessionID ) && is_null( $this->SessionID ) ) && ( !strpos( $command, 'login.anonymously' ) ) && !$this->OAuthSecret ) {
				throw new SMW_PhpSmugException( 'Not authenticated. No Session ID or OAuth Token.  Please login or provide an OAuth token.' );
			}
		}
		...
		$keys = array_map( array( 'SMW_phpSmug', 'urlencodeRFC3986' ), array_keys( $args ) );
		$values = array_map( array( 'SMW_phpSmug', 'urlencodeRFC3986' ), array_values( $args ) );
		...
		if ( $this->parsed_response['stat'] == 'fail' ) {
			...
			throw new SMW_PhpSmugException( "SmugMug API Error for method {$command}: {$this->error_msg}", $this->error_code );
		} else {
	...
	/**
	 * Set a proxy for all SMW_phpSmug calls.
	 *
	 * Params can be passed as an associative array or a set of param=value strings.
	 *
	 * @access	public
	...
	/**
	 * Set Token and Token Secret for use by other methods in SMW_phpSmug.
	 *
	 * Use this method to pull in the token and token secret obtained during 
	 * the OAuth authorisation process.
	 *
	 * If OAuth is being used, this method MUST be called so SMW_phpSmug knows about
	 * the token and token secret.
	 *
	 * NOTE: It's up to the application developer using SMW_phpSmug to store the Access
	 * token and token secret in a location convenient for their application.
	 * SMW_phpSmug can not do this as all storage and caching done by SMW_phpSmug is 
	 ...
	 * entire SMW_phpSmug functionality.  
	 * 
	...
	public function images_upload()
	{
		...
		if ( !array_key_exists( 'File', $args ) ) {
			throw new SMW_PhpSmugException( 'No upload file specified.' );
		}
		...
		// Ensure the FileName is SMW_phpSmug::urlencodeRFC3986 encoded - caters for stange chars and spaces
		$args['FileName'] = SMW_phpSmug::urlencodeRFC3986( $args['FileName'] );
		...
		if ( is_file( $args['File'] ) ) {
			...
			throw new SMW_PhpSmugException( "File doesn't exist: {$args['File']}" );
		}

		// Create a new object as we still need the other request object
		$upload_req = new SMW_httpRequest();
		...
		$upload_req->setHeader( array( 'User-Agent' => "{$this->AppName} using SMW_phpSmug/{$this->version}",
		...
		if ( $this->parsed_response['stat'] == 'fail' ) {
            ...
            $this->error_msg = $this->parsed_response['message'];
			$this->parsed_response = FALSE;
			throw new SMW_PhpSmugException( "SmugMug API Error for method image_upload: {$this->error_msg}", $this->error_code );
		} else {
			$this->error_code = FALSE;
            $this->error_msg = FALSE;
		}
	 ...
	 * not explicitly implemented by SMW_phpSmug.
	 * 
 	 * @access	public
	 * @uses	request
	...
	public function __call( $method, $arguments )
	{
		...
		$args = SMW_phpSmug::processArgs( $arguments );
	 ...
	 public function authorize()
	{
	 ...
	 private function generate_signature( $apicall, $apiargs = NULL )
	 {
		...
		if ( $this->oauth_signature_method == 'PLAINTEXT' ) {
			return SMW_phpSmug::urlencodeRFC3986( $this->OAuthSecret ).'&'.SMW_phpSmug::urlencodeRFC3986( $this->oauth_token_secret );
		} else {
			$this->oauth_signature_method = 'HMAC-SHA1';
			$encKey = SMW_phpSmug::urlencodeRFC3986( $this->OAuthSecret ) . '&' . SMW_phpSmug::urlencodeRFC3986( $this->oauth_token_secret );
		    ...
		    $keys = array_map( array( 'SMW_phpSmug', 'urlencodeRFC3986' ), array_keys( $params ) );
		    $values = array_map( array( 'SMW_phpSmug', 'urlencodeRFC3986' ), array_values( $params ) );
			...
			$base_string = $method . '&' . SMW_phpSmug::urlencodeRFC3986( $endpoint ) . '&' .  SMW_phpSmug::urlencodeRFC3986( $string );
 ...
 * and modified to suit the needs of SMW_phpSmug.
 *
 * The original source is distributed under the Apache License Version 2.0
 */

class SMW_SMW_httpRequestException extends Exception {}

interface SMW_PhpSmugRequestProcessor
{
...
class SMW_httpRequest
{
	...
	private $user_agent = "Unknown application using SMW_phpSmug/3.3";
    ...
    */
	...
	public function __construct( $url = NULL, $method = 'POST', $timeout = 30 )
	{
		...
		if ( function_exists( 'curl_init' ) && ( $this->config['adapter'] == 'curl' )
			...
			$this->processor = new SMW_PhpSmugCurlRequestProcessor;
		}
		else {
			$this->processor = new SMW_PhpSmugSocketRequestProcessor;
		}
	}
...
class SMW_PhpSmugCurlRequestProcessor implements SMW_PhpSmugRequestProcessor
{
	private $response_body = '';
	private $response_headers = '';
	private $executed = FALSE;
	...
	public function execute( $method, $url, $headers, $body, $config )
	{
        ...
        if ( $config['proxy_host'] ) {
            if ( ! $config['proxy_port'] ) {
                throw new SMW_SMW_httpRequestException( 'Proxy port not provided' );
            }
		...
		if ( curl_errno( $ch ) !== 0 ) {
			throw new SMW_SMW_httpRequestException( sprintf( '%s: CURL Error %d: %s', __CLASS__, curl_errno( $ch ), curl_error( $ch ) ), curl_errno( $ch ) );
		}

		if ( substr( curl_getinfo( $ch, CURLINFO_HTTP_CODE ), 0, 1 ) != 2 ) {
			throw new SMW_SMW_httpRequestException( sprintf( 'Bad return code (%1$d) for: %2$s', curl_getinfo( $ch, CURLINFO_HTTP_CODE ), $url ), curl_errno( $ch ) );
		}
...
class SMW_PhpSmugSocketRequestProcessor implements SMW_PhpSmugRequestProcessor
{
	private $response_body = '';
	private $response_headers = '';
	private $executed = FALSE;
